#!/bin/bash
# Converted from PS1: %F{blue}%~%k%f %F{green}${vcs_info_msg_0_}%f
# PS1-style: blue cwd + green git branch (no trailing $)
...
# ---- colors matching PS1 style ----
dir_color() { if [ "$use_color" -eq 1 ]; then printf '\033[34m'; fi; }          # blue (matches %F{blue})
...
# human-readable number formatting (1234 -> 1.2k, 1234567 -> 1.2M)
fmt_num() {
  local n="$1"
  [[ "$n" =~ ^[0-9]+$ ]] || { echo "$n"; return; }
  if (( n >= 1000000 )); then
...
# ---- git colors ----
git_color() { if [ "$use_color" -eq 1 ]; then printf '\033[32m'; fi; }  # green (matches %F{green})
...
get_max_context() {
  ...
  case "$model_name" in
    ...
    *"Opus 4"*|*"opus 4"*|*"Opus"*|*"opus"*)
      echo "200000"  # 200K for standard Opus versions
      ;;
...
if [ -n "$session_id" ] && [ "$HAS_JQ" -eq 1 ]; then
  ...
  if [ -f "$session_file" ]; then
    ...
    if [ -n "$latest_tokens" ] && [ "$latest_tokens" -gt 0 ]; then
      ...
      # Set color based on used percentage (pastel tones)
      if [ "$context_used_pct" -ge 80 ]; then
        context_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;217m'; fi; }  # pastel salmon pink (danger: >80% used)
      elif [ "$context_used_pct" -ge 60 ]; then
        context_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;229m'; fi; }  # pastel cream yellow (caution: 60~80% used)
      else
        context_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;194m'; fi; }  # pastel mint (safe: <60% used)
      fi
  ...
  if [ -n "$blocks_output" ]; then
    ...
    if [ -n "$active_block" ]; then
      ...
      if [ -n "$reset_time_str" ] && [ -n "$start_time_str" ]; then
        ...
        session_txt="$(printf '%dh%dm' "$rh" "$rm")"
        session_bar=$(progress_bar "$session_pct" 5)
      fi
    fi
  fi
fi
...
# ---- render statusline ----
# Line 1: dir + branch + model + context bar + session reset
printf '%s%s%s' "$(dir_color)" "$current_dir" "$(rst)"
if [ -n "$git_branch" ]; then
  printf ' %s(%s)%s' "$(git_color)" "$git_branch" "$(rst)"
fi
model_short=$(echo "$model_name" | sed -E 's/ *\([^)]*\)//g')
printf '  🤖 %s%s%s' "$(model_color)" "$model_short" "$(rst)"
if [ -n "$context_pct" ]; then
  context_bar=$(progress_bar "$context_used_pct" 5)
...
# Caveman mode badge
CAVEMAN_BADGE_SCRIPT="$(dirname "$0")/hooks/caveman-kor/caveman-statusline.sh"
if [ -x "$CAVEMAN_BADGE_SCRIPT" ]; then
  badge=$(bash "$CAVEMAN_BADGE_SCRIPT" 2>/dev/null)
